...
const getEquivalenceStrategy = (kind: ContractKind, code: string) => {
  ...
  return {
    ...
    explanation:
      'This contract appears to use AI/web behaviour, but no obvious equivalence guard was detected.',
  }
}
...
const scanBadPatterns = (code: string, kind: ContractKind, callMap: FrontendCallMapItem[]) => {
  ...
  return [
    ...
    },
    {
      ...
      status: /self\.[A-Za-z_]\w*\s*=\s*gl\.nondet/i.test(code)
        ? 'fail'
    ...
    },
    {
      ...
      status: callMap.some((item) => item.type === 'view') ? 'pass' : 'fail',
  ...
  return {
    ...
    whyGenLayer:
      kind === 'deterministic'
        ...
        : hasAi
        ? 'This belongs on GenLayer because validators agree on an AI-assisted judgement over language, evidence, quality, fraud, moderation, scoring, or another subjective outcome.'
        : 'This belongs on GenLayer because it uses public web/API evidence and needs validators to agree on the evidence and resulting state transition.',
    ...
    equivalenceStrategy: getEquivalenceStrategy(kind, code),
